...
class UtilityController extends Controller
{
     ...
     * - dyComponent[selectedContent] = Sidebar index content that been selected for component at Home content.
     *
     */
    ...
    /**
     * Setting name of home content component view client-side for response at props client-side. Recommended to get value dynamic component by `config('contentenum.value')` function.
     * @param string $name Name of component. If empty string at argument, it will take on default value.
     * @param int|string $selectedContent Content selected by the client side. Make sure it matches the sidebar
     * content on the client side. If empty string at argument, it will take on default value.
     * @param boolean $selfControlNavigation If false, it will prevent self selected content sidebar to navigation.
     * @return void
     */
    public function setDynamicComponent(string $name = '', $selectedContent = '', $selfControlNavigation = false)
    {
        if (empty($name)) {

            if (empty($this->defaultProps['dyComponent']['name']))  throw new ControllerException(ControllerException::NoConfigDynamicComponent, 500);
            else $this->responseProps['dyComponent']['name'] = $name;
        }
        $this->responseProps['dyComponent']['selectedContent'] = empty($selectedContent) ? $this->defaultProps['dyComponent']['selectedContent'] : $selectedContent;
        $this->responseProps['dyComponent']['selfControlNavigation'] = $selfControlNavigation;
    }
     ...
     * - DEPRECATED: If you $useTable value true, it will share your data table at $page variable like https://inertiajs.com/shared-data#accessing-shared-data.
     * - UPDATED: If you $useTable value true, it will share your data table at props of home.
     *
    ...
    protected function configurePropsState()
    {
        ...
        if (empty($this->responseProps)) {
            if (empty($default['dyComponent']['selfControlNavigation'])) {
                $default['selfControlNavigation'] = false;
            }
            return $default;
        } else {
